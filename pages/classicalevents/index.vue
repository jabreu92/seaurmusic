    ...
    <b-container fluid>
      ...
      <section v-else>
        ...
        <div v-else>
          <div v-if="!this.loading">
            <b-row>
              <b-col md="3" v-for="event in classicalEvents" :key="event.id">
                <b-card
    ...
    let url = `https://app.ticketmaster.com/discovery/v2/events?apikey=QLnzwCGhWDMWq3z894HvbEL1QuKH2XGw&source=ticketmaster&locale=*&countryCode=US&stateCode=WA&classificationName=music&classificationId=${genre}`
     ...
     axios
        ...
        .then(response => {
            ...
            this.classicalEvents = this.items._embedded.events

               for (let i =0; i<this.classicalEvents.length; i++){
                let event_image_array = this.classicalEvents[i].images
                let filteredImagesArray = event_image_array.filter(function (el) {
                            return el.width == 640 && el.height == 360;
                 });
                this.classicalEvents[i].images = filteredImagesArray
                let date = new Date(this.classicalEvents[i].dates.start.localDate)
                this.classicalEvents[i].dates.start.localDate = date
